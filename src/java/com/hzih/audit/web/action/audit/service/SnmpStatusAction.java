...
public class SnmpStatusAction extends ActionSupport {
    ...
    public String openServer() throws Exception {
        ...
        try {
            ...
            if (msg_on.contains("is running")) {
                msg = "开启成功";
                logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "服务管理", "开启SNMP服务成功");
            } else {
                msg = "开启失败";
                logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(), "服务管理", "开启SNMP服务失败");
            }
        } catch (Exception e) {
    ...
    public String closeServer() throws IOException {
        ...
        try {
            ...
            if (!msg_on.contains("is running")) {
                msg = "关闭成功";
                logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "服务管理", "关闭SNMP服务成功");
            } else {
                msg = "关闭失败";
                logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(), "服务管理", "关闭SNMP服务失败");
            }
        } catch (Exception e) {
            msg = "关闭失败";
            logger.error(e.getMessage(),e);
    ...
    public String reloadServer() throws IOException {
        ...
        try {
            ...
            try {
                ...
                if (msg_on.contains("is running")) {
                    msg = "重启成功";
                    logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "服务管理", "重启SNMP服务成功");
                } else {
                    msg = "重启失败";
                    logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(), "服务管理", "重启SNMP服务失败");
                }
            } catch (Exception e) {
                logger.error(e.getMessage(),e);
                msg = "重启失败";
