...
import com.hzih.audit.utils.JavaShortMessageUtil;
import com.hzih.audit.utils.MailUtils;
...
//            logService.newLog("INFO",  SessionUtils.getAccount(request).getUserName(), "报警配置","用户查找收件人列表信息成功");
    ...
    public String deleteEmail() throws IOException {
        ...
        try {
            ...
            output.close();
            msg = "删除Email地址成功!";
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "报警配置", "用删除Email地址成功");
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
            logService.newLog("ERROE", SessionUtils.getAccount(request).getUserName(), "报警配置", "用户删除Email地址失败");
    ...
    public String insertEmail() throws IOException {
        ...
        try {
            ...
            output.close();
            msg = "新增Email地址成功!";
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "报警配置", "用户新增Email地址成功");
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
            logService.newLog("ERROE", SessionUtils.getAccount(request).getUserName(), "报警配置", "用户新增Email地址失败");
            msg = "新增Email地址失败!";
        }
        String json = "{success:true,msg:'" + msg + "'}";
        base.actionEnd(response, json, result);
    ...
    public String updateEmail() throws IOException {
        ...
        try {
            ...
            output.close();
            msg = "修改Email地址成功!";
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "报警配置", "用户修改Email地址成功");
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
            logService.newLog("ERROE", SessionUtils.getAccount(request).getUserName(), "报警配置", "用户修改Email地址失败");
