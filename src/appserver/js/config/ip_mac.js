...
function setPermission(){
    ...
    var storeStatus = new Ext.data.SimpleStore({fields:['key','value'],data:dataStatus});

    //应用
    var appRecord = new Ext.data.Record.create([
        {name:'key',	mapping:'key'},
        {name:'value',	mapping:'value'}
    ]);
    var appStore = new Ext.data.GroupingStore({
        proxy : new Ext.data.HttpProxy({ url:'AppTypeAction_readAppNameKeyValue.action?appType=' + appType }),
        reader : new Ext.data.JsonReader({ totalProperty:'total', root:'rows' },appRecord)
    });
    //服务
    ...
    var serverUser_store = new Ext.data.Store({
        url:"ServerUserAction_selectUsersByType.action?serverType=" + serverType  ,
    ...
    if(selModel.hasSelection()){
        ...
        Ext.each(selections,function(item){
            var resourceRecord = Ext.data.Record.create(
                [
                    {name: 'clientServerId',mapping:'clientServerId'},
            ...
            var resourceCm = new Ext.grid.ColumnModel([
                ...
                {header: "应用名", width: 70,align:'center', dataIndex: 'appName',sortable:true,menuDisabled:true},
                ...
                {header: "权限路径", width: 100, dataIndex: 'dir',align:'center',sortable:true,menuDisabled:true},
                {
                    ...
                    width:120,
                    renderer:function(v){
                        return "<a href='javascript:;'  onclick='deletePermission()'>删除</a>";
                    }
                }
            ]);
...
//        bodyStyle:'width:100%',
                ...
                tbar:['服务类型',{
                    ...
                    listeners:{
                        'select':function(){
                            serverType = this.getValue();
                            Ext.getCmp('auth.tbar.serverUser').clearValue();
                            Ext.getCmp('auth.tbar.appName').clearValue();
                            Ext.getCmp('auth.tbar.serverUser').clearValue();
                            appStore.proxy = new Ext.data.HttpProxy({url:'ClientUserAction_readAppNameKeyValue.action?appType=' + serverType});
                            appStore.reload();
                            serverUser_store.proxy = new Ext.data.HttpProxy({url:"ServerUserAction_selectUsersByType.action?serverType=" + serverType});
                            serverUser_store.reload();
                        }
                    }

                },'-','应用',{
                    id:'auth.tbar.appName',
                    fieldLabel:'应用',
                    hiddenName:'appName',
                    xtype:'combo',
                    model:'remote',
...
//                editable:false,
                    ...
                    handler: function(){
                        ...
                        var appName = Ext.getCmp('auth.tbar.appName').getValue();
                        if(serverId > 0 && appName.length > 0){
                            Ext.MessageBox.show({
                                title:'信息',
                                ...
                                fn:function(e){
                                    if(e == 'ok'){
                                        ...
                                        Ext.Ajax.request({
                                            ...
                                            params :{id:serverId,clientId:item.data.id,appName:appName},
                                            method:'POST',
                                            success : function(r,o){
                                                myMask.hide();
                                                var respText = Ext.util.JSON.decode(r.responseText);
                ...
                fn:function(e){
                    if(e == 'ok'){
                        ...
                        Ext.Ajax.request({
                            ...
                            params :{clientServerId:item.data.clientServerId},
                            method:'POST',
                            success : function(r,o){
                                myMask.hide();
                                var respText = Ext.util.JSON.decode(r.responseText);
